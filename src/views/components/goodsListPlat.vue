...
<script>
	...
	import { getPromotion, deletePromotionTargetlistState, savePromotionRangelistBatch, sendPromotionById, queryPromotionRangelistPage,updateDelPromotionRangelist } from "@/api/shopflist";
...
//				this.promotionIdDet = this.addList[0].pprlId
			...
			deletes() {
				this.getPresentDetails.pmPromotionRangeList=null
				this.getPresentDetails.delPmPromotionRangeList = this.addList
//				this.promotionIdDet = this.addList[0].pprlId
				updateDelPromotionRangelist("paramStr=" + encodeURI(JSON.stringify(this.getPresentDetails))).then(res => {
					if(res.success) {
						this.$message.success(res.msg)
...
//				this.addList = se;
			...
			upDates(data) {
				...
				console.log(this.getPresentDetails,'this.getPresentDetails')
				updateDelPromotionRangelist("paramStr=" + encodeURI(JSON.stringify(this.getPresentDetails))).then(res => {
					if(res.success) {
						this.$message.success(res.msg)
//						this.$refs.templateTable.getDataList();
			...
			getPromotion(val) {
				...
				getPromotion({
					...
					this.dataState=res.dataState;
					this.getDate()
				});
			},
			getDate(){
//				console.log(this.dataState,'this.dataState')
				if(this.dataState=='-1'||this.dataState=='2'){
//					this.temInput[this.temInput.length-1].btnArray[1].type=''
					for(let i=0;i<this.temInput[this.temInput.length-1].btnArray.length;i++){
						if(i!=0){
//							this.temInput[this.temInput.length-1].btnArray[i].type='info'
							this.temInput[this.temInput.length-1].btnArray[i].disabled=true
//							console.log(this.temInput[this.temInput.length-1].btnArray,'this.temInput.btnArray')
						}
